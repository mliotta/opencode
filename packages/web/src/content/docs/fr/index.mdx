...
- **Via Homebrew sur macOS et Linux**
  ...
  ```bash
  brew install mliotta/tap/opencode
  ```
...
- **Via Mise**
  ...
  ```bash
  mise use -g github:mliotta/opencode
  ```

- **Via Docker**

  ```bash
  docker run -it --rm ghcr.io/mliotta/opencode
  ```

Le support de l'installation de OpenCode sur Windows à l'aide de Bun est actuellement en cours de développement.

Vous pouvez également récupérer le binaire dans les [Releases](https://github.com/mliotta/opencode/releases).
