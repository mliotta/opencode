      ...
      const handleEvent = Effect.fnUntraced(function* (value: StreamEvent) {
        switch (value.type) {
          ...
          case "tool-result": {
            ...
            yield* car.addFact({
              subject: value.id,
              body: `${output.title || toolCall?.part.tool || "tool"}: ${output.output.slice(0, 500)}`,
              kind: "tool_call",
            })
            return
          }
          ...
          case "tool-error": {
            ...
            yield* car.addFact({
              subject: value.id,
              body: `${toolCall?.part.tool || "tool"} failed: ${value.message.slice(0, 500)}`,
              kind: "tool_error",
            })
            return
          }
          ...
          case "step-finish": {
            ...
            yield* car.addFact({
              ...
              body: (factBody || `(no text, finish: ${value.reason})`).slice(0, 2000),
              kind: "conversation",
            })
            if (ctx.snapshot) {
